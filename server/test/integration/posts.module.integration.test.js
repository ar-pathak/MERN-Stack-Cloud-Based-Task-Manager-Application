...
test("posts protected feed route requires user auth", async () => {
    const result = await requestJson(baseUrl, "/api/posts/feed");
    expectUserAuthRequired(result);
});

test("posts feed rejects malformed access token", async () => {
    const result = await requestJson(baseUrl, "/api/posts/feed", {
        headers: {
...
test("posts feed rejects expired access token", async () => {
