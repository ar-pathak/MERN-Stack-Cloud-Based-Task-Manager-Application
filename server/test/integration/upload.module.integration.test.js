...
test("upload module routes require user auth", async () => {
    const result = await requestJson(baseUrl, "/api/upload/single", {
        method: "POST"
    });
    expectUserAuthRequired(result);
});

test("upload single rejects malformed access token", async () => {
    const result = await requestJson(baseUrl, "/api/upload/single", {
        method: "POST",
        headers: {
...
test("upload single rejects expired access token", async () => {
